...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 bg-white border-b border-neutral-200 transition-shadow ${scrolled ? 'shadow-sm' : ''}`}>
        ...
        <div className="flex items-center h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {menuItems.map((item) => (
              ...
              >
                {item.href ? (
                  <Link to={item.href} className="px-2 lg:px-3 py-2 text-[12px] lg:text-[13px] font-medium text-neutral-700 hover:text-neutral-900 transition-colors flex items-center gap-1">
                    {item.label}
                  </Link>
                ) : (
                  <button className="px-2 lg:px-3 py-2 text-[12px] lg:text-[13px] font-medium text-neutral-700 hover:text-neutral-900 transition-colors flex items-center gap-1 cursor-pointer">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden p-2 hover:bg-neutral-100 rounded-full transition-colors">
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-neutral-200 px-4 py-4 max-h-[70vh] overflow-auto">
          {menuItems.map((item) => (
